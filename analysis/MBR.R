...
LE_2021_mean = MBR_data%>%
  ...
  geom_line(aes(y = mean_daily_LE_Wm2_MBR), color = "darkblue")+
  geom_ribbon(aes(ymin=lower_mean_daily_LE_Wm2_MBR, 
                  ymax=upper_mean_daily_LE_Wm2_MBR), alpha=0.1, fill = "darkblue", 
...
H_2021= slow_profile_data %>%
  ggplot() +
  geom_line(aes(x = datetime,y = H_EC_measured_sonic_30m,color = "MBR")) +
  geom_line(aes(x = datetime,
                y = H_Wm2_Eco,
                color = "EC")) +
  scale_color_manual(name = NULL,values = c(
...
H_2021_mean = slow_profile_data%>%
  group_by(hour(datetime))%>%
  # calculate mean daily values and percentiles
  mutate(mean_daily_H_EC_measured_sonic_30m = mean(H_EC_measured_sonic_30m, na.rm = T), 
         upper_mean_daily_H_EC_measured_sonic_30m = quantile(H_EC_measured_sonic_30m, probs = c(0.75), na.rm = T), 
         lower_mean_daily_H_EC_measured_sonic_30m = quantile(H_EC_measured_sonic_30m, probs = c(0.25), na.rm = T), 
         # for reference Eco data
  ...
  ggplot(aes(x = hour(datetime)))+
  geom_line(aes(y = mean_daily_H_EC_measured_sonic_30m), color = "darkred")+
  geom_ribbon(aes(ymin=lower_mean_daily_H_EC_measured_sonic_30m, 
                  ymax=upper_mean_daily_H_EC_measured_sonic_30m), alpha=0.1, fill = "darkred", 
...
MBR_preds_H_LE = H_2021 + H_2021_mean +
