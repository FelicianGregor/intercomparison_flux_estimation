...
ggplot(ffp_sf) +
  geom_sf(aes(fill = as.factor(percentile)), alpha = 0.3) +
  theme_minimal()
...
ggplot() +
  ...
  # footprint polygons
  geom_sf(data = ffp_wgs84, aes(fill = as.factor(percentile)),
          alpha = 0.4, color = "black", linewidth = 0.2) +
  
  # tower location
  geom_sf(
    data = st_sfc(st_point(c(13.41897, 56.09763)), crs = 4326),
    color = "red", size = 3
  ) +
  
  coord_sf(expand = FALSE) +
  
  scale_fill_viridis_d(name = "Percentile") +
